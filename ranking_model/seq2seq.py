...
def encoder(embeddings, sent_lens, mp):
    encoder_cell = tf.nn.rnn_cell.BasicLSTMCell(mp['hid_size'])

    encoder_outputs, encoder_state = tf.nn.dynamic_rnn(encoder_cell, embeddings,
...
def decoder(embeddings, sent_lens, reply_params,
            ...
            context_params):
    decoder_cell = tf.nn.rnn_cell.BasicLSTMCell(reply_params['hid_size'])
    helper = tf.contrib.seq2seq.TrainingHelper(embeddings, sent_lens)
    ...
    if reply_params.get('attention') == 'bahdanau':
        ...
        initial_state = decoder_cell.zero_state(tf.shape(embeddings)[0], tf.float32)\
            .clone(cell_state=encoder_state)

    decoder = tf.contrib.seq2seq.BasicDecoder(decoder_cell, helper, initial_state)
    _, decoder_state, _ = tf.contrib.seq2seq.dynamic_decode(decoder)
